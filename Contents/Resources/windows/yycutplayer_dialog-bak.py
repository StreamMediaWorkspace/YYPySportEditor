...
class YYCutPlayerDlg(QDialog):
    ...
    def __init__(self, cuts_json, clips_json):
        _ = get_app()._tr
        ...
        self.r = openshot.Timeline(width, height, openshot.Fraction(fps["num"], fps["den"]), sample_rate,
                                          channels, channel_layout)
        self.r.info.channel_layout = channel_layout

        filepath = "/Users/admin/Downloads/BLACKPINK_Kill_This_Love.mp4"

        try:
            # Add clip for current preview file
            self.clip = openshot.Clip(filepath)
            ...
            log.error('Failed to load media file into preview player: %s' % filepath)
            return
